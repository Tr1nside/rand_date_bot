...
| Логирование | `loguru` |
| Линтер | `ruff` + `WPS` (wemake-python-styleguide) |
...
- **Полное следование [wemake-python-styleguide](https://wemake-python-styleguide.rtfd.io/)** — обязательно.
    Все нарушения,которые ловит `ruff --select WPS`, должны быть исправлены до коммита.
...
- **Запрещены локальные импорты** (внутри функций, методов, условных блоков).
- Все импорты размещаются только в начале файла, сгруппированы по стандарту WPS:
  1. Стандартная библиотека
  2. Сторонние пакеты
  3. Локальные модули проекта
- Используй абсолютные импорты внутри проекта (`from bot.services import DateService`), относительные запрещены.

### Файлы `__init__.py`

- **Файлы `__init__.py` всегда должны оставаться пустыми.**
- Запрещено размещать в них:
  - Регистрацию роутеров, хендлеров, мидлварей
  - Импорт и экспорт символов для «удобства»
  - Любую исполняемую логику или инициализацию
- Если требуется регистрация компонентов — делай это явно в `main.py` или в специальном
    модуле `bot/handlers/__init__.py` **не используется**, регистрируй в `main.py`.
